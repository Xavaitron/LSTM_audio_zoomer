...
class SpatialMediumCRN(nn.Module):
    def __init__(self, n_fft=512, hop_length=160):
        ...
        # Decoder (Strict padding=0 ensures 17->33->65->129->257 shape matching)
...
# A. Standard SI-SDR Loss (Adapted from CRN_finetune.py)
...
# B. NEW ENHANCED Multi-Resolution Loss (Power-Law Compressed) from CRN_finetune.py
...
# ==========================================
def main():
    print(f"--- Spatial Audio Training on {DEVICE} (Reverb Fixed) ---")
    ...
    # LOSSES
    criterion_sisdr = SpatialSeparationLoss(n_fft=N_FFT, hop_length=HOP_LENGTH).to(DEVICE)
    ...
    for epoch in range(N_EPOCHS):
        ...
        for mixture, angle, target in loop:
            ...
            loop.set_postfix(sisdr=f"{loss_s.item():.2f}", mr_stft=f"{loss_m.item():.2f}", total=f"{loss.item():.2f}")
